  ...
  fetch('http://localhost:8080/videos')
    ...
    .then(data => {
      if (data.error) {
        ...
        document.getElementById('modalAccounts').innerHTML = 'You have no videos yet. Please save your design to continue later';
      } else {
        let body = '';
        console.log(data)
...
function uploadThumbnails(e) {
  ...
  // link.setAttribute('href', canvasFile.replace("image/png", "image/octet-stream", { multiplier: 2 }));
  // link.click();
  // thumbvids.herokuapp.com
  fetch('http://localhost:8080/uploadThumbnail', {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
